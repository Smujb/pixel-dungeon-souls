	...
	public static class WndGetItem extends Window {
		...
		private static Item getItem(Class itemClass) {
			...
			try {
				...
				e.printStackTrace();
				item = null;
				PDSGame.scene().addToFront(new WndError(e.getMessage()));
		...
		WndGetItem(Item item) {
			...
			RedButton btnChoose = new RedButton( "Input Item" ) {
				...
				protected void onClick() {
					PDSGame.platform.promptTextInput("Enter id of an item you want: ", name(itemClass[0]), Integer.MAX_VALUE, false, "OK", "CANCEL", new PlatformSupport.TextCallback() {
						...
						public void onSelect(boolean positive, String text) {
							if (positive) {
								try {
									...
									PDSGame.scene().addToFront(new WndError(e.getLocalizedMessage()));
								}
								window.update();
							}
						}
	...
	public static class WndChooseDepth extends Window {
		...
		WndChooseDepth(Level level) {
			...
			RedButton btnChoose = new RedButton( "Enter Key" ) {
				@Override
				protected void onClick() {
					PDSGame.platform.promptTextInput("Enter key of level to fetch: ", Random.element(Dungeon.staticLevels.keySet().toArray(new String[0])), Integer.MAX_VALUE, false, "OK", "CANCEL", new PlatformSupport.TextCallback() {
	...
	public void execute(Hero hero, String action) {
		...
		switch (action) {
			...
			case AC_TP:
				...
				break;
			case AC_CHOOSEDEPTH:
				PDSGame.scene().addToFront(new WndChooseDepth(Dungeon.level));
				break;
			case AC_ITEM:
				PDSGame.scene().addToFront(new WndGetItem(this));
				break;
			case AC_KILL:
