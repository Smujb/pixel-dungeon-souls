...
public class WndGameInProgress extends Window {
	...
	public WndGameInProgress(final int slot){
		...
		Button debug = new Button(){
			...
			protected boolean onLongClick() {
				try {
					...
					PDSGame.scene().addToFront(new WndMessage("_Debug Info:_\n\n" +
		...
		statSlot( Messages.get(this, "souls"), info.souls );
		...
		RedButton erase = new RedButton( Messages.get(this, "erase")){
			...
			protected void onClick() {
				...
				PDSGame.scene().add(new WndOptions(
					...
					protected void onSelect( int index ) {
						if (index == 0) {
							...
							PDSGame.switchNoFade(StartScene.class);
						}
					}
				} );
			}
