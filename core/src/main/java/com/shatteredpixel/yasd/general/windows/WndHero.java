	...
	private class AbilitiesTab extends Group {
		...
		AbilitiesTab() {
			...
			RedButton btnInfo = new RedButton(Messages.get(this, "info")) {
				@Override
				protected void onClick() {
					PDSGame.scene().addToFront(new WndTitledMessage( HeroSprite.avatar(hero.heroClass, 6 ), Messages.get(AbilitiesTab.class, "info_title"), Messages.get(AbilitiesTab.class, "info_desc")));
				}
			};
